...
运行测试，可以看到，我们期望得到400的响应，实际上却成功了。

#### 完善逻辑

我们使用`Validation`框架进行校验。首先加上校验逻辑：
...
运行测试，失败——`user_id` 还是写死的 1。首先我们获取到当前用户：
...
重新运行测试，测试通过。一般而言，当我们某个测试是测试登录用户的某个行为时，通常会有一个相对应的测试：测试未登录用户不能进行该行为。
