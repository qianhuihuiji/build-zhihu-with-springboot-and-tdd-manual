...
新建测试 *src/test/java/com/nofirst/spring/tdd/zhihu/startup/integration/answers/UpVotesTest.java*：
...
这个测试会直接通过——Spring Security 拦截了未登录请求。
...
运行测试，显示路由不存在，那我们就来添加对应的controller和路由：
...
再次运行测试，我们没有写任何逻辑，当然测试会失败。我们补充完整逻辑：
