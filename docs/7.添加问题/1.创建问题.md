...
从本节开始，用户终于可以自己提问了——之前一直是运营手动导入数据。
...
再次测试，测试并未通过。可以看到，我们给`question`定义了一个`categoryId`的属性。这是一个新的概念：分类，我们接下来先添加`category`表：
...
运行全部测试，发现有很多测试失败了。但是经过观察就会发现，大多数的错误都是：
